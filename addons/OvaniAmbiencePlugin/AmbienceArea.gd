...
func _fadeInto():
	...
	_currentTween.set_parallel(true)
	print("entering " + name)
			...
			for j in AudioServer.get_bus_effect_count(busIdx):
				...
				for prop in effect.get_property_list():
					if prop.type in [TYPE_FLOAT, TYPE_INT, TYPE_BOOL]:
						if snap.Effects[j] is AudioEffectLowPassFilter && prop.name == "cutoff_hz":
							_currentTween.set_trans(Tween.TRANS_EXPO)
							if effect.cutoff_hz < snap.Effects[j]["cutoff_hz"]:
								_currentTween.set_ease(Tween.EASE_IN)
							else:
								_currentTween.set_ease(Tween.EASE_OUT)
						else:
							_currentTween.set_trans(Tween.TRANS_LINEAR)
